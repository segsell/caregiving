...
def enforce_model_choice_restriction(df, specs):
    ...
    max_ret_age_empirical = specs["max_ret_age_empirical"]
    ...
    df = df[
        ~((~df["choice"].isin(retired_values)) & (df["age"] >= max_ret_age_empirical))
    ]
    ...
    print(str(len(df)) + " left after dropping who are unemployed after min SRA.")
