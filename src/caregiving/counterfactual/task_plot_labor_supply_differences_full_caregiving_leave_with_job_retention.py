"""Plot differences in labor supply for full-caregiving-leave-with-job-retention.

Counterfactual.

Compares full-caregiving-leave-with-job-retention counterfactual vs:
1. Baseline scenario
"""
...
@pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
def task_plot_matched_differences_by_distance_caregiving_leave(  # noqa: PLR0915, E501
    path_to_full_cg_leave_data: Path = BLD
    / "solve_and_simulate"
    ...
    / "all"
    / "all"
    / "matched_differences_by_distance_full_caregiving_leave_with_job_retention.png",
    ...
    Compares full-cg-leave-jr vs baseline.
    """
    ...
    df_full_cg, df_baseline = prepare_dataframes_for_comparison(
        pd.read_pickle(path_to_full_cg_leave_data),
        pd.read_pickle(path_to_baseline_data),
        ever_caregivers=ever_caregivers,
    )

    # Calculate outcomes
    full_cg_work, full_cg_ft, full_cg_pt = calculate_simple_outcomes(
        df_full_cg, "job_retention"
    )
    ...
    full_cg_additional = calculate_additional_outcomes(df_full_cg, specs)
    baseline_additional = calculate_additional_outcomes(df_baseline, specs)

    # Outcome columns
    full_cg_cols = df_full_cg[["agent", "period"]].copy()
    full_cg_cols["work_cg"] = full_cg_work
    full_cg_cols["ft_cg"] = full_cg_ft
    full_cg_cols["pt_cg"] = full_cg_pt
    full_cg_cols["gross_labor_income_cg"] = full_cg_additional["gross_labor_income"]
    full_cg_cols["savings_cg"] = full_cg_additional["savings"]
    full_cg_cols["wealth_cg"] = full_cg_additional["wealth"]
    full_cg_cols["savings_rate_cg"] = full_cg_additional["savings_rate"]
    full_cg_cols["consumption_cg"] = full_cg_additional["consumption"]
    full_cg_cols["bequest_from_parent_cg"] = full_cg_additional["bequest_from_parent"]
    if "caregiving_leave_top_up" in full_cg_additional:
        full_cg_cols["caregiving_leave_top_up_cg"] = full_cg_additional[
    ...
    baseline_cols["consumption_baseline"] = baseline_additional["consumption"]
    baseline_cols["bequest_from_parent_baseline"] = baseline_additional[
        "bequest_from_parent"
    ]

    # Add education and caregiving_type columns for filtering
    if "education" in df_full_cg.columns:
        full_cg_cols["education"] = df_full_cg["education"].values
    if "caregiving_type" in df_full_cg.columns:
        full_cg_cols["caregiving_type"] = df_full_cg["caregiving_type"].values

    # Differences (full-cg-leave - baseline)
    merged = full_cg_cols.merge(baseline_cols, on=["agent", "period"], how="inner")
    merged["diff_work"] = merged["work_cg"] - merged["work_baseline"]
    merged["diff_ft"] = merged["ft_cg"] - merged["ft_baseline"]
    merged["diff_pt"] = merged["pt_cg"] - merged["pt_baseline"]
    merged["diff_gross_labor_income"] = (
        merged["gross_labor_income_cg"] - merged["gross_labor_income_baseline"]
    )
    merged["diff_savings"] = merged["savings_cg"] - merged["savings_baseline"]
    merged["diff_wealth"] = merged["wealth_cg"] - merged["wealth_baseline"]
    merged["diff_savings_rate"] = (
        merged["savings_rate_cg"] - merged["savings_rate_baseline"]
    )
    merged["diff_consumption"] = (
        merged["consumption_cg"] - merged["consumption_baseline"]
    )
    merged["diff_bequest_from_parent"] = (
        merged["bequest_from_parent_cg"] - merged["bequest_from_parent_baseline"]
    )
    ...
    df_full_cg_dist = _add_distance_to_first_care(df_full_cg)
    dist_map = (
        df_full_cg_dist.groupby("agent", observed=False)["first_care_period"]
        .first()
        .reset_index()
    )
    ...
    for _edu_label, edu_value, edu_dir in education_specs:
        for (
            ...
            if full_cg_type_value is not None:
                merged_filtered = merged_filtered[
                    merged_filtered["caregiving_type"] == full_cg_type_value
                ]
            ...
            # We need: .../matched_differences/{edu_dir}/{full_cg_type_dir}/filename.png
            base_path = path_to_plot.parent.parent.parent / edu_dir / full_cg_type_dir
            base_path.mkdir(parents=True, exist_ok=True)
            plot_path = base_path / (
                "matched_differences_by_distance_"
                "full_caregiving_leave_with_job_retention.png"
            )

            # Save plot for this subgroup
            plot_three_line_differences(
                prof=prof,
            ...
            # Also save to declared product path for the all/all case
            if edu_dir == full_cg_type_dir == "all":
...
@pytask.mark.counterfactual_differences
@pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
def task_plot_matched_differences_by_age_at_first_care_full_cg_leave(  # noqa: PLR0912, PLR0915
    # noqa: E501
    path_to_full_cg_leave_data: Path = BLD
    / "solve_and_simulate"
    / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
    ...
    full_cg_outcomes = calculate_outcomes(df_full_cg, choice_set_type="original")
    baseline_outcomes = calculate_outcomes(df_baseline, choice_set_type="original")

    specs = pickle.load(path_to_specs.open("rb"))
    full_cg_outcomes["hours_weekly"] = calculate_working_hours_weekly(
        df_full_cg, specs, choice_set_type="original"
    )
    ...
    full_cg_additional = calculate_additional_outcomes(df_full_cg, specs)
    baseline_additional = calculate_additional_outcomes(df_baseline, specs)
    full_cg_outcomes.update(full_cg_additional)
    baseline_outcomes.update(baseline_additional)

    full_cg_cols = create_outcome_columns(df_full_cg, full_cg_outcomes, "_o")
    ...
    if "caregiving_leave_top_up" in full_cg_outcomes:
    ...
    caregiving_mask = df_full_cg["choice"].isin(care_codes)
    first_care_with_age = get_age_at_first_event(
        df_full_cg, caregiving_mask, "age_at_first_care"
    )
        ...
        for (
            ...
            if "diff_caregiving_leave_top_up" in merged_filtered.columns:
                ...
                plot_configs["caregiving_leave_top_up"] = {
                    "path": (
                        ...
                        / full_cg_type_dir
                        / (
                            "matched_differences_caregiving_leave_top_up_by_age_"
                            "at_first_care.png"
                        )
                    ),
                    "ylabel": (
            ...
            if edu_dir == full_cg_type_dir == "all":
                declared_paths_map = {
                    ...
                    "hours_weekly": path_to_plot_working_hours,
                }
...
# @pytask.mark.counterfactual_differences
# @pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
# def task_plot_matched_differences_by_age_bins_at_first_care_full_cg_leave(
#  # noqa: PLR0915, E501
#     path_to_full_cg_leave_data: Path = BLD
#     / "solve_and_simulate"
#     / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
...
#     full_cg_outcomes = calculate_outcomes(df_full_cg, choice_set_type="original")
#     baseline_outcomes = calculate_outcomes(df_baseline, choice_set_type="baseline")
...
#     )
#     baseline_outcomes["hours_weekly"] = calculate_working_hours_weekly(
#         df_baseline, specs, choice_set_type="baseline"
#     )
...
#                         / full_cg_type_dir
#                         / (
#                             "matched_differences_caregiving_leave_top_up_by_age"
#                             "_bins_at_first_care.png"
#                         ),
#                     ),
# "ylabel": "Caregiving Leave Top-up
# (Monthly)\nDeviation from No Care Demand",
#                     "diff_col": "diff_caregiving_leave_top_up",
#                 }
#             else:
...
# @pytask.mark.counterfactual_differences
# @pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
# def task_plot_matched_differences_by_age_at_first_care_demand_full_cg_leave
# (  # noqa: PLR0915, E501
#     path_to_full_cg_leave_data: Path = BLD
#     / "solve_and_simulate"
#     / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
...
#     df_full_cg_dist = _add_distance_to_first_care_demand(df_full_cg)
#     dist_map = (
#         df_full_cg_dist.groupby("agent", observed=False)["first_care_demand_period"]
#         .first()
#         .reset_index()
#     )

#     care_demand_mask = df_full_cg["care_demand"] > 0
#     first_care_demand_with_age = get_age_at_first_event(
#         df_full_cg, care_demand_mask, "age_at_first_care_demand"
#     )
...
# merged = merged[merged["age_at_first_care_demand"].isin(ages_at_first_care_
# demand)]
...
#                 "path": base_path
#                 / "matched_differences_care_by_age_at_first_care_demand.png",
# "ylabel": "Probability of Providing
# Care\nDeviation from No Care Demand",
#                 "diff_col": "diff_care",
#                 "xlabel": "Year relative to first care demand",
#             },
...
#                 "path": base_path
#                 / (
#                     "matched_differences_gross_labor_income_by_age_at_first_"
#                     f"care_demand.png"
#                 ),
# "ylabel": "Gross Labor Income
# (Monthly)\nDeviation from No Care Demand",
...
#                 "path": base_path
#                 / (
#                     "matched_differences_bequest_from_parent_by_age_at_first_"
#                     f"care_demand.png"
#                 ),
#                 "ylabel": "Bequest from Parent\nDeviation from No Care Demand",
#                 "diff_col": "diff_bequest_from_parent",
...
# @pytask.mark.counterfactual_differences
# @pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
# def task_plot_matched_differences_by_age_bins_at_first_care_demand_full_cg_
# leave(  # noqa: PLR0915, E501
#     path_to_full_cg_leave_data: Path = BLD
#     / "solve_and_simulate"
#     / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
#     path_to_baseline_data: Path = BLD
...
#                 "path": base_path
#                 / (
#                     "matched_differences_employment_rate_by_age_bins_at_"
#                     f"first_care_demand.png"
#                 ),
#                 "ylabel": "Proportion Working\nDeviation from No Care Demand",
#                 "diff_col": "diff_work",
#                 "xlabel": "Year relative to first care demand",
...
#                 "path": base_path
#                 / (
#                     "matched_differences_working_hours_by_age_bins_at_"
#                     f"first_care_demand.png"
#                 ),
# "ylabel": "Weekly Working Hours
# Difference\nDeviation from No Care Demand",
...
#                 "path": base_path
#                 / (
#                     "matched_differences_gross_labor_income_by_age_bins_"
#                     f"at_first_care_demand.png"
#                 ),
# "ylabel": "Gross Labor Income
# (Monthly)\nDeviation from No Care Demand",
...
#                 "path": base_path
#                 / (
#                     "matched_differences_bequest_from_parent_by_age_bins_"
#                     f"at_first_care_demand.png"
#                 ),
#                 "ylabel": "Bequest from Parent\nDeviation from No Care Demand",
#                 "diff_col": "diff_bequest_from_parent",
...
#                 / (
#                     "matched_differences_caregiving_leave_top_up_by_age_"
#                     f"bins_at_first_care_demand.png"
#                 )
#                     ),
# "ylabel": "Caregiving Leave Top-up
# (Monthly)\nDeviation from No Care Demand",
...
#             )
# prof =
# prof.sort_values(["age_bin_start", "distance_to_first_care_demand"])
...
# @pytask.mark.counterfactual_differences
# @pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
# def task_plot_matched_differences_by_distance_to_first_care_all_outcomes(
# # noqa: PLR0915, E501
#     path_to_full_cg_leave_data: Path = BLD
#     / "solve_and_simulate"
#     / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
...
#                     "path": base_path
#                 / (
#                     "matched_differences_caregiving_leave_top_up_by_distance_"
#                     f"to_first_care.png"
#                 ),
# "ylabel": "Caregiving Leave Top-up
# (Monthly)\nDeviation from No Care Demand",
...
# @pytask.mark.counterfactual_differences
# @pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
# def task_plot_matched_differences_by_distance_to_first_care_demand_all_outc
# omes(  # noqa: PLR0915, E501
#     path_to_full_cg_leave_data: Path = BLD
#     / "solve_and_simulate"
#     / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
#     path_to_baseline_data: Path = BLD
...
#                     "path": base_path
#                 / (
#                     "matched_differences_gross_labor_income_by_distance_"
#                     f"to_first_care_demand.png"
#                 ),
# "ylabel": "Gross Labor Income
# (Monthly)\nDeviation from No Care Demand",
#                     "diff_col": "diff_gross_labor_income",
...
#                     "path": base_path
#                 / (
#                     "matched_differences_bequest_from_parent_by_distance_"
#                     f"to_first_care_demand.png"
#                 ),
# "ylabel": "Bequest from Parent (in
# 1,000€)\nDeviation from No Care Demand",
...
@pytask.mark.counterfactual_differences
@pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
def task_plot_matched_differences_by_distance_to_first_care_all_outcomes_vs_baseline(  # noqa: PLR0912, PLR0915
    # noqa: E501
    path_to_full_cg_leave_data: Path = BLD
    / "solve_and_simulate"
    / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
        ...
        for (
            ...
            if "diff_caregiving_leave_top_up" in prof.columns:
                plot_configs["caregiving_leave_top_up"] = {
                    "path": base_path
                    / (
                        "matched_differences_caregiving_leave_top_up_by_distance_to_first_"
                        "care.png"
                    ),
                    "ylabel": (
                        "Caregiving Leave Top-up (Monthly)\n" "Deviation from Baseline"
            ...
            # Map plot config keys to declared product paths for "all/all" case
            declared_paths = {
                "work": path_to_plot_work,
                "ft": path_to_plot_ft,
                "pt": path_to_plot_pt,
                ...
                "consumption": path_to_plot_consumption,
            }

            # Update plot_configs paths for "all/all" case to use declared product paths
            if edu_dir == full_cg_type_dir == "all":
                for outcome_key, declared_path in declared_paths.items():
                    if outcome_key in plot_configs:
                        plot_configs[outcome_key]["path"] = declared_path
        ...
        for (
            ...
            plot_configs = {
                ...
                "gross_labor_income": {
                    "path": base_path
                    / (
                        "matched_differences_gross_labor_income_by_distance_to_first_care_"
                        "demand.png"
                    ),
                    "ylabel": "Gross Labor Income (Monthly)\nDeviation from Baseline",
                    "diff_col": "diff_gross_labor_income",
                ...
                "savings": {
                    "path": (
                        base_path
                        / (
                            "matched_differences_savings_by_distance_to_"
                ...
                "bequest_from_parent": {
                    "path": base_path
                    / (
                        "matched_differences_bequest_from_parent_by_distance_to_first_care_"
                        "demand.png"
                    ),
                    "ylabel": (
                        "Bequest from Parent (in 1,000€)\n" "Deviation from Baseline"
            ...
            if "diff_caregiving_leave_top_up" in prof.columns:
                plot_configs["caregiving_leave_top_up"] = {
                    ...
                    / (
                        # noqa: E501
                        "matched_differences_caregiving_leave_top_up_by_distance_to_first_care_"  # noqa: E501
                        "demand.png"
                    ),
                    "ylabel": (
                        "Caregiving Leave Top-up (Monthly)\n" "Deviation from Baseline"
            ...
            # Plot each outcome separately
            for outcome_key, config in plot_configs.items():
                diff_col = config["diff_col"]
                if diff_col not in prof.columns:
                    continue
                ...
                plt.tight_layout()
                plt.savefig(config["path"], dpi=300, bbox_inches="tight")
                # Also save to declared product path for "all/all" case
                if (
                    edu_dir == full_cg_type_dir == "all"
                    and outcome_key in declared_paths
                ):
                    declared_paths[outcome_key].parent.mkdir(
                        parents=True, exist_ok=True
                    )
                    plt.savefig(
                        declared_paths[outcome_key], dpi=300, bbox_inches="tight"
                    )
                plt.close()
...
@pytask.mark.counterfactual_differences
@pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
def task_plot_matched_differences_by_age_bins_at_first_care_full_cg_leave_vs_baseline(  # noqa: PLR0912, PLR0915
    # noqa: E501
    path_to_full_cg_leave_data: Path = BLD
    / "solve_and_simulate"
    / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
        ...
        return {
            ...
            "gross_labor_income": {
                "path": (
                    base_path
                    / (
                        "matched_differences_gross_labor_income_by_age_"
                        "bins_at_first_care.png"
                    )
                ),
                "ylabel": "Gross Labor Income (Monthly)\nDeviation from Baseline",
                "diff_col": "diff_gross_labor_income",
            ...
            "bequest_from_parent": {
                "path": (
                    base_path
                    / (
                        "matched_differences_bequest_from_parent_by_age_"
                        "bins_at_first_care.png"
                    )
                ),
                "ylabel": "Bequest from Parent\nDeviation from Baseline",
            ...
            if "diff_caregiving_leave_top_up" in merged_filtered.columns:
                ...
                plot_configs["caregiving_leave_top_up"] = {
                    "path": (
                        ...
                        / full_cg_type_dir
                        / (
                            "matched_differences_caregiving_leave_top_up_by_age_bins_at_first_"
                            "care.png"
                        )
                    ),
                    "ylabel": (
...
@pytask.mark.counterfactual_differences
@pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
def task_plot_matched_differences_by_age_at_first_care_demand_full_cg_leave_vs_baseline(  # noqa: PLR0912, PLR0915
    # noqa: E501
    path_to_full_cg_leave_data: Path = BLD
    / "solve_and_simulate"
    / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
    ...
    def create_plot_configs(base_path):
        ...
        return {
            ...
            "bequest_from_parent": {
                "path": base_path
                / (
                    "matched_differences_bequest_from_parent_by_age_at_first_care_"
                    "demand.png"
                ),
                "ylabel": "Bequest from Parent\nDeviation from Baseline",
                "diff_col": "diff_bequest_from_parent",
            ...
            if "diff_caregiving_leave_top_up" in merged_filtered.columns:
                ...
                plot_configs["caregiving_leave_top_up"] = {
                    "path": (
                        ...
                        / full_cg_type_dir
                        / (
                            "matched_differences_caregiving_leave_top_up_by_age_at_first_care_"
                            "demand.png"
                        )
                    ),
                    "ylabel": (
...
@pytask.mark.counterfactual_differences
@pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
def task_plot_matched_differences_by_age_bins_at_first_care_demand_full_cg_leave_vs_baseline(  # noqa: PLR0912, PLR0915, E501
    # noqa: E501
    path_to_full_cg_leave_data: Path = BLD
    / "solve_and_simulate"
    / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
    ...
    def create_plot_configs(base_path):
        ...
        return {
            ...
            "work": {
                "path": base_path
                / (
                    "matched_differences_employment_rate_by_age_bins_at_first_care_"
                    "demand.png"
                ),
                "ylabel": "Proportion Working\nDeviation from Baseline",
                "diff_col": "diff_work",
            ...
            "gross_labor_income": {
                "path": base_path
                / (
                    "matched_differences_gross_labor_income_by_age_bins_at_first_care_"
                    "demand.png"
                ),
                "ylabel": "Gross Labor Income (Monthly)\nDeviation from Baseline",
                "diff_col": "diff_gross_labor_income",
            ...
            "savings_rate": {
                "path": (
                    base_path
                    / (
                        "matched_differences_savings_rate_by_age_bins_"
                        "at_first_care_demand.png"
                    )
                ),
                "ylabel": "Savings Rate\nDeviation from Baseline",
                "diff_col": "diff_savings_rate",
            ...
            "consumption": {
                "path": (
                    base_path
                    / (
                        "matched_differences_consumption_by_age_bins_"
                        "at_first_care_demand.png"
                    )
                ),
                "ylabel": "Consumption\nDeviation from Baseline",
            ...
            "bequest_from_parent": {
                "path": base_path
                / (
                    "matched_differences_bequest_from_parent_by_age_bins_at_first_care_"
                    "demand.png"
                ),
                "ylabel": "Bequest from Parent\nDeviation from Baseline",
                "diff_col": "diff_bequest_from_parent",
            ...
            if "diff_caregiving_leave_top_up" in merged_filtered.columns:
                ...
                plot_configs["caregiving_leave_top_up"] = {
                    "path": (
                        ...
                        / (
                            # noqa: E501
                            "matched_differences_caregiving_leave_top_up_by_age_bins_at_first_care_"  # noqa: E501
                            "demand.png"
                        )
                    ),
                    "ylabel": (
...
@pytask.mark.counterfactual_differences
@pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
def task_plot_matched_differences_first_care_start_by_age_full_cg_leave(  # noqa: PLR0915
    # noqa: E501
    path_to_full_cg_leave_data: Path = BLD
    / "solve_and_simulate"
    / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
    ...
    if "age" not in df_full_cg.columns or "age" not in df_baseline.columns:
        raise ValueError("Age column required but not found in data")

    # Find first care period for each agent
    care_codes = np.asarray(INFORMAL_CARE).ravel().tolist()

    # For full_cg_leave - include education and caregiving_type if available
    caregiving_mask_cg = df_full_cg["choice"].isin(care_codes)
    ...
    first_care_cg = (
        df_full_cg.loc[caregiving_mask_cg, cols_to_select]
        .sort_values(["agent", "period"])
        .drop_duplicates("agent", keep="first")
        .rename(columns={"period": "first_care_period", "age": "age_at_first_care"})
    )

    # For baseline (should be empty, but calculate for consistency)
    caregiving_mask_ncd = df_baseline["choice"].isin(care_codes)
    first_care_ncd = (
        df_baseline.loc[caregiving_mask_ncd, ["agent", "period", "age"]]
        .sort_values(["agent", "period"])
        .drop_duplicates("agent", keep="first")
        .rename(columns={"period": "first_care_period", "age": "age_at_first_care"})
    )
    ...
    for edu_label, edu_value, edu_dir in education_specs:
        for (
            ...
            first_care_full_cg_filtered = first_care_cg.copy()
            if (
                edu_value is not None
                and "education" in first_care_full_cg_filtered.columns
            ):
                first_care_full_cg_filtered = first_care_full_cg_filtered[
                    first_care_full_cg_filtered["education"] == edu_value
                ]
            if (
                full_cg_type_value is not None
                and "caregiving_type" in first_care_full_cg_filtered.columns
            ):
                first_care_full_cg_filtered = first_care_full_cg_filtered[
                    first_care_full_cg_filtered["caregiving_type"] == full_cg_type_value
                ]

            # Count by age
            counts_cg = (
                first_care_full_cg_filtered["age_at_first_care"]
                .value_counts()
                .sort_index()
            ...
            title_suffix = (
                f" ({edu_label}, {full_cg_type_label})"
                if (edu_label != "all" or full_cg_type_label != "all")
                else ""
            )
            plt.title(
            ...
            if edu_dir == full_cg_type_dir == "all":
                path_to_plot.parent.mkdir(parents=True, exist_ok=True)
                # Re-save the same figure to the declared path
...
@pytask.mark.counterfactual_differences
@pytask.mark.counterfactual_differences_full_caregiving_leave_with_job_retention
def task_plot_matched_differences_first_care_demand_start_by_age_full_cg_leave(  # noqa: PLR0912, PLR0915
    # noqa: E501
    path_to_full_cg_leave_data: Path = BLD
    / "solve_and_simulate"
    / "simulated_data_full_caregiving_leave_with_job_retention_estimated_params.pkl",
    ...
    # # Find first period where care_demand > 0 for each agent - include educatio
    # n and caregiving_type if available
    care_demand_mask_cg = df_full_cg["care_demand"] > 0
    cols_to_select = ["agent", "period", "age"]
    if "education" in df_full_cg.columns:
        cols_to_select.append("education")
    if "caregiving_type" in df_full_cg.columns:
    ...
    first_care_demand_cg = (
        df_full_cg.loc[care_demand_mask_cg, cols_to_select]
        .sort_values(["agent", "period"])
        .drop_duplicates("agent", keep="first")
        .rename(
            columns={
    ...
    # For baseline scenario, there is no care_demand column,
    # so create empty dataframe
    if "care_demand" in df_baseline.columns:
        care_demand_mask_ncd = df_baseline["care_demand"] > 0
        first_care_demand_ncd = (
            df_baseline.loc[care_demand_mask_ncd, ["agent", "period", "age"]]
            .sort_values(["agent", "period"])
            .drop_duplicates("agent", keep="first")
            .rename(
                columns={
    ...
    for edu_label, edu_value, edu_dir in education_specs:
        for (
            ...
            first_care_demand_full_cg_filtered = first_care_demand_cg.copy()
            if (
                edu_value is not None
                and "education" in first_care_demand_full_cg_filtered.columns
            ):
                first_care_demand_full_cg_filtered = first_care_demand_full_cg_filtered[
                    first_care_demand_full_cg_filtered["education"] == edu_value
                ]
            if (
                full_cg_type_value is not None
                and "caregiving_type" in first_care_demand_full_cg_filtered.columns
            ):
                first_care_demand_full_cg_filtered = first_care_demand_full_cg_filtered[
                    first_care_demand_full_cg_filtered["caregiving_type"]
                    == full_cg_type_value
                ]

            # Count by age
            if len(first_care_demand_full_cg_filtered) > 0:
                counts_cg = (
                    first_care_demand_full_cg_filtered["age_at_first_care_demand"]
                    .value_counts()
                    .sort_index()
                    .reset_index(name="count_cg")
                    .rename(columns={"age_at_first_care_demand": "age"})
