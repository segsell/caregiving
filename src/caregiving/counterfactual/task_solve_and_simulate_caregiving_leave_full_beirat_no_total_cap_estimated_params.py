"""Solve and simulate the Full-Beirat-no-total-cap caregiving leave counterfactual.

Same eligibility/benefits as the Full Beirat variant, but the 3-yr cumulative cap
is removed; the 1-yr full-leave sub-cap and indefinite job retention are kept.
See ``task_specify_model_caregiving_leave_full_beirat_no_total_cap`` and
POLICY_DATA_SOURCES.md sec 10 for the design.
"""

import pickle
from pathlib import Path
from typing import Annotated

import jax
import yaml
from pytask import Product

import dcegm
from caregiving.config import BLD
from caregiving.model.shared import DEAD
from caregiving.model.state_space_caregiving_leave_beirat import (
    create_state_space_functions_full_beirat_no_total_cap,
)
from caregiving.model.task_specify_model_caregiving_leave_full_beirat_no_total_cap import (  # noqa: E501
    create_stochastic_states_transitions,
)
from caregiving.model.taste_shocks import shock_function_dict
from caregiving.model.utility.bequest_utility import (
    create_final_period_utility_functions,
)
from caregiving.model.utility.utility_functions_additive import create_utility_functions
from caregiving.model.wealth_and_budget.budget_equation_caregiving_leave_full_beirat_no_total_cap import (  # noqa: E501
    budget_constraint,
)
from caregiving.simulation.simulate import create_additional_variables

jax.config.update("jax_enable_x64", True)


def solve_and_simulate_caregiving_leave_full_beirat_no_total_cap_estimated_params(
    path_to_specs: Path = BLD / "model" / "specs" / "specs_full.pkl",
    path_to_model: Path = BLD
    / "model"
    / "model_caregiving_leave_full_beirat_no_total_cap.pkl",
    path_to_model_config: Path = BLD
    / "model"
    / "model_config_caregiving_leave_full_beirat_no_total_cap.pkl",
    path_to_estimated_params: Path = BLD
    / "model"
    / "params"
    / "estimated_params_model.yaml",
    path_to_initial_states: Path = (
        BLD
        / "model"
        / "initial_conditions"
        / "initial_states_full_beirat_no_total_cap.pkl"
    ),
    path_to_save_solution: Annotated[Path, Product] = BLD
    / "solve_and_simulate"
    / "solution_caregiving_leave_full_beirat_no_total_cap_estimated_params.pkl",
    path_to_save_simulated_data: Annotated[Path, Product] = BLD
    / "solve_and_simulate"
    / "simulated_data_caregiving_leave_full_beirat_no_total_cap_estimated_params.pkl",
) -> None:
    """Solve and simulate the Full-Beirat-no-total-cap counterfactual."""
    specs = pickle.load(path_to_specs.open("rb"))
    model_config = pickle.load(path_to_model_config.open("rb"))
    params = yaml.safe_load(path_to_estimated_params.open("rb"))

    model = dcegm.setup_model(
        model_specs=specs,
        model_config=model_config,
        state_space_functions=create_state_space_functions_full_beirat_no_total_cap(),
        utility_functions=create_utility_functions(),
        utility_functions_final_period=create_final_period_utility_functions(),
        budget_constraint=budget_constraint,
        shock_functions=shock_function_dict(),
        stochastic_states_transitions=create_stochastic_states_transitions(),
        model_load_path=path_to_model,
    )
    model_solved = model.solve(params, save_sol_path=path_to_save_solution)

    initial_states = pickle.load(path_to_initial_states.open("rb"))

    sim_df = model_solved.simulate(
        states_initial=initial_states,
        seed=specs["seed"],
    )

    sim_df = sim_df[sim_df["health"] != DEAD].copy()
    sim_df.reset_index(inplace=True)

    sim_df = create_additional_variables(sim_df, specs)

    sim_df.to_pickle(path_to_save_simulated_data)
