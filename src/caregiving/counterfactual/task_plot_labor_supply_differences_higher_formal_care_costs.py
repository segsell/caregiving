"""Plot differences in labor supply for higher formal care costs counterfactual.

Compares higher formal care costs counterfactual vs:
...
def task_plot_matched_differences_by_distance(  # noqa: PLR0915
    ...
    / "vs_no_care_demand"
    / "matched_differences_by_distance_higher_formal_care_costs.png",
    ever_caregivers: bool = True,
    window: int = 20,
) -> None:
    """Compute matched period differences (higher-formal-care-costs - no-care-demand).
    ...
    Steps:
      ...
      5) Compute distance_to_first_care from higher formal care costs, attach to merged.
      6) Average diffs by distance and plot three series.

    """
    # Load and prepare data
    df_hfc, df_ncd = prepare_dataframes_simple(
    ...
    # Calculate outcomes
    hfc_work, hfc_ft, hfc_pt = calculate_simple_outcomes(df_hfc, "original")
    ncd_work, ncd_ft, ncd_pt = calculate_simple_outcomes(df_ncd, "no_care_demand")

    # Create outcome columns
    hfc_cols = df_hfc[["agent", "period"]].copy()
    hfc_cols["work_hfc"] = hfc_work
    hfc_cols["ft_hfc"] = hfc_ft
    hfc_cols["pt_hfc"] = hfc_pt
    ...
    # Merge and compute differences (higher formal care costs - no care demand)
    merged = hfc_cols.merge(ncd_cols, on=["agent", "period"], how="inner")
    merged["diff_work"] = merged["work_hfc"] - merged["work_ncd"]
    merged["diff_ft"] = merged["ft_hfc"] - merged["ft_ncd"]
...
def task_plot_matched_differences_by_age_at_first_care(  # noqa: PLR0915
    ...
    Compares higher formal care costs vs no care demand counterfactuals.
    ...
    Steps:
      ...
      4) Merge on (agent, period) and compute differences.
       5) Compute distance_to_first_care and age_at_first_care from higher
          formal care costs.
    ...
    caregiving_mask = df_hfc["choice"].isin(care_codes)
    first_care_with_age = get_age_at_first_event(
        df_hfc, caregiving_mask, "age_at_first_care"
    )
...
def task_plot_matched_differences_by_distance_vs_baseline(  # noqa: PLR0915
    ...
    """Compute matched period differences (higher-formal-care-costs - baseline).
    ...
    """
    # Load and prepare data
    df_hfc, df_baseline = prepare_dataframes_simple(
        pd.read_pickle(path_to_higher_formal_care_costs_data),
    ...
    # Merge and compute differences (higher formal care costs - baseline)
    merged = hfc_cols.merge(baseline_cols, on=["agent", "period"], how="inner")
    merged["diff_work"] = merged["work_hfc"] - merged["work_baseline"]
    merged["diff_ft"] = merged["ft_hfc"] - merged["ft_baseline"]
...
def task_plot_matched_differences_by_age_at_first_care_vs_baseline(  # noqa: PLR0915
    ...
    Compares higher formal care costs vs baseline counterfactuals.
    Creates separate plots for part-time and full-time work, with separate lines
    for each age at which caregiving started.
...
def task_plot_matched_differences_by_distance_by_care_demand(  # noqa: PLR0915
    ...
    / "higher_formal_care_costs"
    / "vs_no_care_demand"
    / "matched_differences_by_distance_by_care_demand_higher_formal_care_costs.png",
    ...
    Steps:
      ...
      4) Merge on (agent, period) and compute differences.
       5) Compute distance_to_first_care_demand from higher formal care costs,
          attach to merged.
...
def task_plot_matched_differences_by_age_at_first_care_demand(  # noqa: PLR0915
    ...
    care_demand_mask = df_hfc["care_demand"] > 0
    first_care_demand_with_age = get_age_at_first_event(
        df_hfc, care_demand_mask, "age_at_first_care_demand"
    )
...
@pytask.mark.counterfactual_differences
@pytask.mark.counterfactual_differences_higher_formal_care_costs
def task_plot_matched_differences_first_care_start_by_age_higher_formal_care_costs(  # noqa: PLR0915
    path_to_higher_formal_care_costs_data: Path = BLD
    / "solve_and_simulate"
    / "simulated_data_higher_formal_care_costs_estimated_params.pkl",
    path_to_no_care_demand_data: Path = BLD
    ...
    """Plot matched differences in distribution of first care start ages (Higher Formal Care Costs vs no care)."""
    ...
    plt.ylabel(
        "Difference in number of people\n(Higher Formal Care Costs - No Care Demand)",
        fontsize=16,
    )
...
@pytask.mark.counterfactual_differences
@pytask.mark.counterfactual_differences_higher_formal_care_costs
def task_plot_matched_differences_first_care_demand_start_by_age_higher_formal_care_costs(  # noqa: PLR0915
    path_to_higher_formal_care_costs_data: Path = BLD
    / "solve_and_simulate"
    / "simulated_data_higher_formal_care_costs_estimated_params.pkl",
    path_to_no_care_demand_data: Path = BLD
    ...
    """Plot matched differences in distribution of first care demand start ages (Higher Formal Care Costs vs no care)."""
    from caregiving.model.shared import DEAD
