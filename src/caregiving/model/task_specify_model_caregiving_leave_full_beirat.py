"""Specify model for full Beirat caregiving leave (max 3 years, max 1 year full leave).

Implements the Pflegebeirat recommendation: total leave cap 3 years, of which
at most 1 can be full leave (not working while caregiving); up to 3 years
partial leave if no full leave is taken. Baseline Pflegegeld kept alongside
the 65% leave top-up. Compare with task_specify_model_caregiving_leave_beirat
(partial leave only, no full leave).
"""
...
from caregiving.model.state_space_caregiving_leave_beirat import (
    create_state_space_functions_full_beirat,
)
...
from caregiving.model.wealth_and_budget.budget_equation_caregiving_leave_full_beirat import (  # noqa: E501
    budget_constraint,
)
from caregiving.model.wealth_and_budget.savings_grid import create_end_of_period_assets


@pytask.mark.caregiving_leave_full_beirat_model
def task_specify_model_caregiving_leave_full_beirat(
    path_to_derived_specs: Path = BLD / "model" / "specs" / "specs_full.pkl",
    path_to_save_model_config: Annotated[Path, Product] = BLD
    / "model"
    / "model_config_caregiving_leave_full_beirat.pkl",
...
def specify_model_caregiving_leave_full_beirat(
    path_to_derived_specs: Path = BLD / "model" / "specs" / "specs_full.pkl",
    path_to_save_model_config: Annotated[Path, Product] = BLD
    / "model"
    / "model_config_caregiving_leave_full_beirat.pkl",
    ...
    model = dcegm.setup_model(
        ...
        state_space_functions=create_state_space_functions_full_beirat(),
    ...
    print("Full Beirat caregiving leave model specified.", flush=True)

    return model


def create_stochastic_states_transitions():
    """Stochastic state transitions for full Beirat model."""
    return {
