"""Specify model for lower formal care costs counterfactual."""
...
def task_specify_model_lower_formal_care_costs(
    ...
    / "model"
    / "options_lower_formal_care_costs.pkl",
    path_to_save_model: Annotated[Path, Product] = BLD
    / "model"
    / "model_lower_formal_care_costs.pkl",
    path_to_save_start_params: Annotated[Path, Product] = BLD
    / "model"
    / "params"
    / "start_params_model_lower_formal_care_costs.yaml",
):
    """Generate model and options for lower formal care costs counterfactual.

    This counterfactual implements a policy where formal care costs are reduced by 50%
    (multiplied by 0.5), making formal care more affordable.
    """
    ...
    print("Lower formal care costs counterfactual model specified.")
    return model, params
