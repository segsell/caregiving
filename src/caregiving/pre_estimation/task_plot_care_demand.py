...
def task_plot_care_demand_transition(  # noqa: PLR0915
    ...
    categories: No Care Demand (Dead), No Care Demand (Alive), Light Care,
    and Intensive Care.
    ...
    n_care_demand_states = 4  # NO_CARE_DEMAND_DEAD, NO_CARE_DEMAND_ALIVE, CARE_DEMAND_LIGHT, CARE_DEMAND_INTENSIVE

    n_agents = len(mother_dead_initial)

    # Map initial conditions to format expected by care_demand_death_transition_light_intensive
    # Initial format:
    # - mother_dead: 0=alive, 1=dead
    # - mother_adl: 0=No ADL, 1=ADL 1, 2=ADL 2/3 (only for alive mothers; dead mothers have ADL=0)
    #
    # Expected format for care_demand_death_transition_light_intensive:
    # - mother_adl: 0=dead, 1=No ADL alive, 2=ADL 1, 3=ADL 2/3
    mother_adl_mapped = np.zeros(n_agents, dtype=np.uint8)
    alive_mask = mother_dead_initial == 0
    dead_mask = mother_dead_initial == 1
    # If dead: mother_adl_mapped = 0 (dead state)
    # If alive: map ADL states: 0->1, 1->2, 2->3
    mother_adl_mapped[alive_mask] = mother_adl_initial[alive_mask] + 1
    mother_adl_mapped[dead_mask] = 0  # Dead state

    # Convert ADL states to care demand states at period 0 (ignoring caregiving window)
    # Direct mapping: ADL states -> care demand states
    care_demand_initial = np.zeros(n_agents, dtype=np.uint8)
    care_demand_initial[mother_adl_mapped == 0] = NO_CARE_DEMAND_DEAD  # Dead
    care_demand_initial[mother_adl_mapped == 1] = NO_CARE_DEMAND_ALIVE  # No ADL alive
    care_demand_initial[mother_adl_mapped == 2] = CARE_DEMAND_LIGHT  # ADL 1
    care_demand_initial[mother_adl_mapped == 3] = CARE_DEMAND_INTENSIVE  # ADL 2/3

    # Initialize share arrays
    share_no_care_dead = np.zeros(n_periods)
    share_no_care_alive = np.zeros(n_periods)
    share_light = np.zeros(n_periods)
    share_intensive = np.zeros(n_periods)
    share_any_care = np.zeros(n_periods)

    # Initial shares from care demand states
    share_no_care_dead[0] = np.mean(care_demand_initial == NO_CARE_DEMAND_DEAD)
    share_no_care_alive[0] = np.mean(care_demand_initial == NO_CARE_DEMAND_ALIVE)
    share_light[0] = np.mean(care_demand_initial == CARE_DEMAND_LIGHT)
    share_intensive[0] = np.mean(care_demand_initial == CARE_DEMAND_INTENSIVE)
    share_any_care[0] = share_light[0] + share_intensive[0]

    # Track ADL state counts by group (edu, mother_adl_state) for transitions
    # The transition function expects ADL states (0=dead, 1=no ADL alive, 2=ADL 1, 3=ADL 2/3)
    adl_by_group = {}
    for edu in range(n_edu):
        for adl_state in range(
            4
        ):  # 4 ADL states: 0=dead, 1=no ADL alive, 2=ADL 1, 3=ADL 2/3
            mask = (education == edu) & (mother_adl_mapped == adl_state)
            adl_by_group[(edu, adl_state)] = float(mask.sum())

    # Simulate forward deterministically using ADL and death transitions
    # We use the same logic as care_demand_death_transition_light_intensive but track ADL states
    for period in range(1, n_periods):
        adl_next = {}
        for edu in range(n_edu):
            # Handle dead mothers (ADL state 0) - they stay dead
            count_dead = adl_by_group.get((edu, 0), 0.0)
            if count_dead > 0:
                adl_next[(edu, 0)] = count_dead

            # Handle alive mothers (ADL states 1, 2, 3)
            for adl_curr_state in range(1, 4):  # ADL states 1, 2, 3 (alive)
                ...
                # death_transition expects mother_dead (0=alive), returns [alive_prob, dead_prob]
                ...
                # Count who dies
                count_dies = count_curr * dead_prob
                count_survives = count_curr * alive_prob

                # Add newly dead to ADL state 0
                adl_next[(edu, 0)] = adl_next.get((edu, 0), 0.0) + count_dies

                if count_survives == 0:
                    continue

                # Get ADL transition probabilities for survivors
                # Map ADL state format: 1->0, 2->1, 3->2 for limitations_with_adl_transition
                adl_state_for_transition = adl_curr_state - 1
                adl_prob_vector = limitations_with_adl_transition(
                    adl_state_for_transition, period - 1, edu, specs
                )
                # adl_prob_vector: [prob_no_adl, prob_adl_1, prob_adl_2/3]

                # Distribute survivors to next ADL states
                ...
                adl_next[(edu, 1)] = adl_next.get((edu, 1), 0.0) + count_no_adl
                adl_next[(edu, 2)] = adl_next.get((edu, 2), 0.0) + count_adl_1
                adl_next[(edu, 3)] = adl_next.get((edu, 3), 0.0) + count_adl_2_3

        # Update ADL counts
        adl_by_group = adl_next

        # Convert ADL states to care demand states (ignoring caregiving window)
        # Direct mapping: ADL states -> care demand states
        total_no_care_dead = sum(
            adl_by_group.get((edu, 0), 0.0)
            for edu in range(n_edu)  # ADL state 0 = dead
        )
        total_no_care_alive = sum(
            adl_by_group.get((edu, 1), 0.0)
            for edu in range(n_edu)  # ADL state 1 = no ADL alive
        )
        total_light = sum(
            adl_by_group.get((edu, 2), 0.0)
            for edu in range(n_edu)  # ADL state 2 = ADL 1
        )
        total_intensive = sum(
            adl_by_group.get((edu, 3), 0.0)
            for edu in range(n_edu)  # ADL state 3 = ADL 2/3
        )

        # Normalize by total agents
        share_no_care_dead[period] = total_no_care_dead / n_agents
        share_no_care_alive[period] = total_no_care_alive / n_agents
...
@pytask.mark.pre_estimation
def task_plot_care_demand_transition_adl_and_dead_state_no_death_state(  # noqa: PLR0915
    path_to_states: Path = BLD / "model" / "initial_conditions" / "initial_states.pkl",
    path_to_full_specs: Path = BLD / "model" / "specs" / "specs_full.pkl",
    path_to_save: Annotated[Path, Product] = BLD
    / "plots"
    / "pre_estimation"
    / "care_demand_by_age_no_death_state.png",
):
    """Plot share of care demand states by age using only care_demand_death_transition_light_intensive.

    Uses the 4-state ADL system (0=dead, 1=no ADL alive, 2=ADL 1, 3=ADL 2/3).
    The 4-state ADL vector is updated internally by care_demand_death_transition_light_intensive,
    which handles all transitions (death and ADL) internally. No other transition functions are called.
    ...
    n_edu = specs["n_education_types"]
    n_adl_states_4 = 4  # 0=dead, 1=no ADL alive, 2=ADL 1, 3=ADL 2/3
    n_care_demand_states = 4  # 0=dead, 1=no care alive, 2=light, 3=intensive

    n_agents = len(mother_dead_initial)

    # Map initial conditions to 4-state ADL system
    # Initial format: mother_dead (0=alive, 1=dead), mother_adl (0/1/2 for alive)
    # Target format: mother_adl_4state (0=dead, 1=no ADL alive, 2=ADL 1, 3=ADL 2/3)
    mother_adl_4state = np.zeros(n_agents, dtype=np.uint8)
    alive_mask = mother_dead_initial == 0
    dead_mask = mother_dead_initial == 1
    # If dead: mother_adl_4state = 0
    # If alive: map ADL states: 0->1, 1->2, 2->3
    mother_adl_4state[alive_mask] = mother_adl_initial[alive_mask] + 1
    mother_adl_4state[dead_mask] = 0
    ...
    # Compute initial care demand shares using care_demand_death_transition_light_intensive
    care_demand_by_group_initial = {}
    for edu in range(n_edu):
        for adl_4state in range(n_adl_states_4):
            count = np.sum((education == edu) & (mother_adl_4state == adl_4state))
            if count == 0:
                continue

            # Get care demand probabilities
            care_demand_probs = care_demand_death_transition_light_intensive(
                adl_4state, 0, edu, specs
            )
    ...
    # Initial shares
    total_no_care_dead = sum(
        care_demand_by_group_initial.get((edu, NO_CARE_DEMAND_DEAD), 0.0)
        for edu in range(n_edu)
    )
    total_no_care_alive = sum(
        care_demand_by_group_initial.get((edu, NO_CARE_DEMAND_ALIVE), 0.0)
        for edu in range(n_edu)
    )
    total_light = sum(
    ...
    share_no_care_dead[0] = total_no_care_dead / n_agents
    share_no_care_alive[0] = total_no_care_alive / n_agents
    share_light[0] = total_light / n_agents
    share_intensive[0] = total_intensive / n_agents
    share_any_care[0] = share_light[0] + share_intensive[0]

    # Track 4-state ADL system by group (edu, adl_4state)
    adl_4state_by_group = {}
    for edu in range(n_edu):
        for adl_4state in range(n_adl_states_4):
            mask = (education == edu) & (mother_adl_4state == adl_4state)
            adl_4state_by_group[(edu, adl_4state)] = float(mask.sum())

    # Simulate forward using ONLY care_demand_death_transition_light_intensive
    # This function handles all transitions internally (death and ADL)
    for period in range(1, n_periods):
        adl_4state_next = {}
        care_demand_by_group = {}

        for edu in range(n_edu):
            for adl_4state_curr in range(n_adl_states_4):
                count_curr = adl_4state_by_group.get((edu, adl_4state_curr), 0.0)
                if count_curr == 0:
                    continue

                # Get care demand probabilities (which internally handle all transitions)
                # Returns [p_dead, p_no_care_alive, p_light, p_intensive]
                care_demand_probs = care_demand_death_transition_light_intensive(
                    adl_4state_curr, period - 1, edu, specs
                )

                # Map care demand probabilities to next period's 4-state ADL system
                # The care demand states map to ADL states as follows:
                # - NO_CARE_DEMAND_DEAD (0) -> ADL state 0 (dead)
                # - NO_CARE_DEMAND_ALIVE (1) -> ADL state 1 (no ADL alive)
                # - CARE_DEMAND_LIGHT (2) -> ADL state 2 (ADL 1)
                # - CARE_DEMAND_INTENSIVE (3) -> ADL state 3 (ADL 2/3)
                # Note: This mapping works because care_demand_death_transition_light_intensive
                # internally uses limitations_with_adl_transition and death_transition

                prob_dead = float(care_demand_probs[NO_CARE_DEMAND_DEAD])
                prob_no_care_alive = float(care_demand_probs[NO_CARE_DEMAND_ALIVE])
                prob_light = float(care_demand_probs[CARE_DEMAND_LIGHT])
                prob_intensive = float(care_demand_probs[CARE_DEMAND_INTENSIVE])

                # Update next period ADL states
                adl_4state_next[(edu, 0)] = (
                    adl_4state_next.get((edu, 0), 0.0) + count_curr * prob_dead
                )
                adl_4state_next[(edu, 1)] = (
                    adl_4state_next.get((edu, 1), 0.0) + count_curr * prob_no_care_alive
                )
                adl_4state_next[(edu, 2)] = (
                    adl_4state_next.get((edu, 2), 0.0) + count_curr * prob_light
                )
                adl_4state_next[(edu, 3)] = (
                    adl_4state_next.get((edu, 3), 0.0) + count_curr * prob_intensive
                )

                # Also track care demand for plotting
                for care_demand_state in range(n_care_demand_states):
                    prob = float(care_demand_probs[care_demand_state])
                    key = (edu, care_demand_state)
                    care_demand_by_group[key] = (
                        care_demand_by_group.get(key, 0.0) + count_curr * prob
                    )

        # Update ADL 4-state counts for next iteration
        adl_4state_by_group = adl_4state_next

        # Calculate total shares
        total_no_care_dead = sum(
            care_demand_by_group.get((edu, NO_CARE_DEMAND_DEAD), 0.0)
            for edu in range(n_edu)
        )
        total_no_care_alive = sum(
            care_demand_by_group.get((edu, NO_CARE_DEMAND_ALIVE), 0.0)
            for edu in range(n_edu)
        )
        total_light = sum(
    ...
    ax.set_title(
        "Share of Care Demand States by Age\n"
        "(using care_demand_death_transition_light_intensive with 4-state ADL)"
    )
    ax.legend(title="Care level", fontsize=9, title_fontsize=10, loc="best")
    ...
    print(f"Care demand by age plot (no death state) saved to {path_to_save}")
