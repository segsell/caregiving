...
def task_plot_inheritance_probability_no_care_demand(
    ...
    Note: The inheritance transition function computes the probability of receiving
    inheritance conditional on the mother being recently dead (PARENT_RECENTLY_DEAD).
    Although mother_dead is not an explicit parameter of the function, inheritance
    can only occur when the mother is recently dead, so this probability is implicitly
    conditional on that state.

    This plot should match (approximately, allowing for sampling variation) the
    post-estimation plot that shows the share of gets_inheritance == 1 conditional
    on mother_dead == PARENT_RECENTLY_DEAD from simulated data.
