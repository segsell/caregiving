...
@pytask.mark.model_fit_caregiving_leave_with_job_retention
def task_plot_model_fit_caregiving_leave_with_job_retention(  # noqa: PLR0915
    ...
    caregiving leave with job retention model implements a policy where
    caregivers can take leave while receiving wage replacement and retain
    their job even when they reduce hours or become unemployed due to
    ...
    df_emp_wealth = create_df_wealth(
        df_full=df_emp_wealth, specs=specs, model_class=model_class
    )
    plot_wealth_by_age_and_education(
