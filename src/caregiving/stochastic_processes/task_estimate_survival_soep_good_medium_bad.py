...
def task_estimate_mortality(
    ...
    """Estimate the mortality matrix without conditioning on education.

    Health now has three states (e.g. Bad, Medium, Good).
    """
    ...
    # Expand the life table to include all combinations of health (3 states) and sex (2 states).
    # Here we assume that health is coded as 0,1,2 and sex as 0 (male) and 1 (female).
    mortality_df = pd.DataFrame(
        [
            ...
            for (health, sex) in itertools.product(range(3), range(2))
        ]
    )
    mortality_df.reset_index(drop=True, inplace=True)

    # Plain life table data (without health) as provided by the statistical office.
    ...
    # Add an intercept column to both df and start_df.
    df["intercept"] = 1
    start_df["intercept"] = 1

    # Loop over sexes (e.g. 0=men, 1=women)
    for sex, sex_label in enumerate(specs["sex_labels"]):
        ...
        # Filter estimation sample by sex.
    ...
    mortality_df = mortality_df[["age", "sex", "health", "death_prob"]]
    lifetable_df = lifetable_df[["age", "sex", "death_prob"]]

    mortality_df = mortality_df.astype(
        {"age": "int", "sex": "int", "health": "int", "death_prob": "float"}
    )
    lifetable_df = lifetable_df.astype(
        {"age": "int", "sex": "int", "death_prob": "float"}
    )
    mortality_df.to_csv(path_to_save_mortality_transition_matrix, sep=",", index=False)
    lifetable_df.to_csv(path_to_save_lifetable, sep=",", index=False)
...
#         # algo_options = {"stopping_maxiter": 100}
...
#         # res = em.estimate_ml(
#         #     loglike=loglike_estimagic,
#         #     params=initial_params,
#         #     optimize_options={
#         #         "algorithm": "fides",
#         #         "algo_options": {"stopping_maxiter": 100},
#         #     },
#         #     loglike_kwargs={"data": filtered_df, "start_data": filtered_start_df},
#         #     # numdiff_options=om.NumdiffOptions(n_cores=4),
#         #     # multistart=om.MultistartOptions(n_samples=100, seed=0, n_cores=4),
#         # )
