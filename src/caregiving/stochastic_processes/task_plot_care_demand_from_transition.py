...
#     path_to_save_plot: Path | None = None,
# ) -> None:
#     """Alternative plot using ADL-by-age from parent_child_data and survival tables.
...
#     - P(ADL | alive, mother age) from ``parent_child_data.csv``,

#     plus the ADL transition matrix and care-demand transition.
#     """
#     df = _expected_care_demand_share_by_age_from_health_tables(
#         options=options,
#         path_to_parent_child_data=path_to_parent_child_data,
...
#     # Use mother_health if available, otherwise use mother_adl (with mapping)
#     if "mother_health" in states:
#         mother_health_agents = np.asarray(states["mother_health"])
#     else:
#         # Map mother_adl to mother_health: ADL states 0-3 map to health states 0-2 (Bad/Medium/Good)
#         # Death (ADL 4) maps to health 3 (Death)
#         mother_adl_agents = np.asarray(states["mother_adl"])
#         # Simple mapping: ADL 0,1 -> Health 2 (Good), ADL 2 -> Health 1 (Medium), ADL 3 -> Health 0 (Bad), ADL 4 -> Health 3 (Death)
#         # This is a rough approximation - you may want to refine this mapping
#         mother_health_agents = np.where(
#             mother_adl_agents == 4, 3, np.clip(3 - mother_adl_agents, 0, 2)
#         )
...
#                     # Map ADL state to health state for care_demand_and_supply_transition
#                     # ADL 0,1 -> Health 2 (Good), ADL 2 -> Health 1 (Medium), ADL 3 -> Health 0 (Bad), ADL 4 -> Health 3 (Death)
#                     if mother_adl_state == 4:
#                         mother_health_state = 3  # Death
#                     else:
#                         mother_health_state = np.clip(3 - mother_adl_state, 0, 2)
...
#     education_values: Iterable[int] = (0, 1),
# ) -> pd.DataFrame:
#     """Compute expected share using ADL-by-age from parent_child_data and survival tables.

#     This uses the parent_child_data to get ADL distributions by age (similar to
#     initial conditions), then propagates forward using the ADL transition matrix.
#     """
#     specs = options["model_params"]
...
#     # Get ADL distributions by age from parent_child_data (for mothers only)
#     mothers_data = parent_child_data[parent_child_data["sex"] == MOTHER].copy()
...
#     adl_prob_by_age = adl_prob_by_age[[0, 1, 2, 3]]  # Ensure order

#     min_age = int(adl_prob_by_age.index.min())
#     max_age = int(adl_prob_by_age.index.max())
...
#             # Get initial ADL distribution at start age
#             mother_start_age = int(round(start_age_child + diff))
#             if mother_start_age not in adl_prob_by_age.index:
#                 continue

#             # Get ADL probabilities (conditional on alive) and survival probability
#             adl_probs_alive = adl_prob_by_age.loc[mother_start_age].to_numpy()
#             prob_alive = (
#                 survival_by_age.loc[mother_start_age]
#                 if mother_start_age in survival_by_age.index
#                 else 1.0
#             )

#             # Combine: P(ADL state | alive) * P(alive) for states 0-3, P(dead) for state 4
#             pi = np.zeros(5, dtype=float)
#             pi[:4] = adl_probs_alive * prob_alive
#             pi[4] = 1.0 - prob_alive

#             for age_child in ages_child:
#                 period = age_child - start_age_child
#                 mother_period = (
#                     period + (specs["start_age"] - specs["start_age_parents"]) + diff
#                 )
#                 n_periods = adl_trans_mat.shape[1]
#                 mother_period = np.clip(mother_period, 0, n_periods - 1)

#                 # Compute expected care demand using current ADL distribution
#                 expected_share = 0.0
#                 for mother_adl_state, weight in enumerate(pi):
#                     if weight == 0:
#                         continue
...
#                 # Update ADL distribution for next period
#                 new_pi = np.zeros_like(pi)
#                 for mother_adl_state, weight in enumerate(pi):
#                     if weight == 0:
#                         continue
