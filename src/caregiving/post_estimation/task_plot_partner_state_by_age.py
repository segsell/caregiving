...
Creates a plot showing the share (proportion) of agents in each partner state category:
- No partner (Single)
- Working partner
- Retired partner
...
def task_plot_partner_state_by_age_baseline(
    ...
    df_sim["no_partner"] = (df_sim["partner_state"] == 0).astype(float)
    df_sim["working_partner"] = (df_sim["partner_state"] == 1).astype(float)
    df_sim["retired_partner"] = (df_sim["partner_state"] == 2).astype(float)
    ...
    for edu_var, edu_label in enumerate(education_labels):
        ...
        if len(df_edu) > 0:
            for partner_idx, (partner_col, partner_label) in enumerate(
                zip(partner_state_cols, partner_labels)
            ):
